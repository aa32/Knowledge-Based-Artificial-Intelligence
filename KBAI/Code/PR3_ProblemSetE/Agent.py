...
#End referenced code
    ...
    def pixelCompare2(self, imgA,imgB,imgC,imgD,imgE,imgF,imgG,imgH,img1,img2,img3,img4,img5,img6,img7,img8,imgs,prob_img):
        ...
        ###Horizontal affine C = A Affine B#######
        AB_AND =  ImageChops.logical_or(imgA1, imgB1)
        #AB_AND.show()
        AB_AND_pix = self.gP(AB_AND)
        img_diffi = np.sum(np.equal(AB_AND_pix,pixC))/pixA.size
        #print("img_diffi",img_diffi)
        if (img_diffi >0.98):
            tp = []
            GH_AND =  ImageChops.logical_or(imgG1, imgH1)
            GH_AND_pix =  self.gP(GH_AND)
            for i,element in enumerate(pix_res):
                diffi =  np.sum(np.equal(GH_AND_pix,pix_res[i]))/pixA.size
                tp.append(diffi)
            print("tp",tp)
            ov_e = np.max(tp) 
            ov_ind = np.argmax(tp)+1
             
            print("ov_ind",ov_ind)
            
        
        ###Horizontal affine A = B Affine C#######
        BC_AND =  ImageChops.logical_or(imgB1, imgC1)
        #AB_AND.show()
        BC_AND_pix = self.gP(BC_AND)
        img_diffi1 = np.sum(np.equal(BC_AND_pix,pixA))/pixA.size
        #print("img_diffi1",img_diffi1)
        if (img_diffi1 >0.98):
            tp = []
            
            G1pix =  self.gP(imgG1)
            for i,element in enumerate(pix_res):
                IH_AND =  ImageChops.logical_or(img_res1[i], imgH1)
                IH_AND_pix = self.gP(IH_AND)
                diffi1 =  np.sum(np.equal(IH_AND_pix,G1pix))/pixA.size
                tp.append(diffi1)
            #print("tp",tp)
            ov_e1 = np.max(tp) 
            ov_ind1 = np.argmax(tp)+1
             
            print("ov_ind1",ov_ind1)
            #    
            
            
        
        AB_or =  ImageChops.logical_or(imgA1, imgB1)
        AB_xor =  ImageChops.logical_xor(imgA1, imgB1)
        AB_multiply = ImageChops.multiply(imgA1, imgB1)
        
        pix_AB_AND = self.gP(AB_AND)
        pix_AB_or = self.gP(AB_or)
        pix_AB_xor = self.gP(AB_xor)
        pix_AB_multiply = self.gP(AB_multiply)
        
        darkpix_AB_AND      = np.sum(pix_AB_AND)
        darkpix_AB_or       = np.sum(pix_AB_or)
        darkpix_AB_xor      = np.sum(pix_AB_xor)
        darkpix_AB_multiply = np.sum(pix_AB_multiply)
        
      
        
        affinetrans = []
        
        affinetrans.append(abs(darkpixC-darkpix_AB_AND))
        affinetrans.append(abs(darkpixC-darkpix_AB_or))
        affinetrans.append(abs(darkpixC-darkpix_AB_xor))
        affinetrans.append(abs(darkpixC-darkpix_AB_multiply))
        min_affine_diff = np.argmin(affinetrans)
        min_affine_val = np.min(affinetrans)
        print("min affine val",min_affine_val)
        print("min_affine_diff", min_affine_diff)
        
        
        if(min_affine_val < 51):
            #print("i am in affine")
            if (min_affine_diff == 0):
                print("it is AND")
                GH_affine = ImageChops.logical_and(imgG1, imgH1)
                pixGH_affine = self.gP(GH_affine)
                
            
            elif (min_affine_diff == 1):
                print("it is or")
                GH_affine = ImageChops.logical_or(imgG1, imgH1)
                pixGH_affine = self.gP(GH_affine)
        
            elif (min_affine_diff == 2):
                print("it is xor")
                GH_affine = ImageChops.logical_or(imgG1, imgH1)
                pixGH_affine = self.gP(GH_affine)
            
            elif (min_affine_diff == 3):
                print("it is multiply")
                GH_affine = ImageChops.multiply(imgG1, imgH1)
                pixGH_affine = self.gP(GH_affine)
            ...
            for i, element in enumerate(pix_res):    
                equalpixelcountHI = np.sum(np.equal(pix_res[i], pixGH_affine).astype(int))/pixG.size
                pix_similar.append(equalpixelcountHI)
                
            q = np.argmax(pix_similar)+1
            #print("Minimum affine values",q,min_affine_val)
        else :
            q = 0
        ...
        #####Start : Cross Diagnal Affine###########
        AF_AND =  ImageChops.logical_and(imgA1, imgF1)
        EG_AND =  ImageChops.logical_and(imgE1, imgG1)
        #EG_AND.show()
        
    
        
        pix_AF_AND = self.gP(AF_AND)
        pix_EG_AND = self.gP(EG_AND)
       
        
        darkpix_AF_AND      = np.sum(pix_AF_AND)
        darkpix_EG_AND      = np.sum(pix_EG_AND)
        
        CEG_diff = abs(darkpixC-darkpix_EG_AND)
        HAF_diff =abs (darkpixH-darkpix_AF_AND)
      
        #print("CEG_diff",CEG_diff)
        #print("HAF_diff", HAF_diff)
        
        s =-1
        min_cross = 1000
        if(CEG_diff < 56 or HAF_diff < 56):
            print("i am in cross diagnal affine")
            
            DBI_cross_diff = []
         
            for i, element in enumerate(imgs): 
                BI_AND =  ImageChops.logical_and(imgB1, img_res1[i])
                pix_BI_AND = self.gP(BI_AND)
                darkpix_BI_AND      = np.sum(pix_BI_AND)
                DBI_diff = abs(darkpixD-darkpix_BI_AND)
                DBI_cross_diff.append(DBI_diff)
                
                
            s = np.argmin(DBI_cross_diff)+1
            min_cross = np.min(DBI_cross_diff)
        
        
        
        #print("Minimum cross affine values",s,min_cross)
        ...
        print("all_ratio_ADG_v",all_ratio_ADG_v)
        ...
        diff_ADG_BEH = abs(all_ratio_ADG_v-all_ratio_BEH_v)
        dark_CFI_ratio_diff = []
        diff_all_ind_v =-1
        if(diff_ADG_BEH < 0.002):
           
            for i, element in enumerate(darkpix_res):
                    sum_dark_CFI_v = darkpixC+darkpixF+darkpix_res[i]
                    ratio = sum_dark_CFI_v/sum_all_v
                    diff_ADG_CFI = abs(all_ratio_ADG_v-ratio)
                    dark_CFI_ratio_diff.append(diff_ADG_CFI)
                   
            #diff_all_diff = abs(dark_CFI_ratio_diff - diff_ADG_BEH)
            print("dark_CFI_ratio_diff",dark_CFI_ratio_diff)
            diff_all_v = np.min(dark_CFI_ratio_diff)
            if(diff_all_v < 0.002)  :
                diff_all_ind_v  = np.argmin(dark_CFI_ratio_diff)+1
